...
describe('System Benchmark: W3C ACT Rules Validation (Alfa)', () => {
    ...
    benchmarkData.testcases.splice(345).forEach((tc: W3CActTestCaseType) => {
        it(`Alfa: ${tc.ruleId} - ${tc.testcaseTitle}`, () => {
            ...
            cy.document()
                ...
                .then((alfaResult) => {
                    ...
                    const isExpectedClean =
                        tc.expected === 'passed' ||
                        tc.expected === 'inapplicable'
                    const isOutcomeClean =
                        outcome === 'passed' || outcome === 'inapplicable'

                    if (tc.expected === 'failed') {
                        expect(
                            outcome,
                            `ACT Rule expected a failure, but Alfa reported: ${outcome}`
                        ).to.equal('failed')
                    } else if (isExpectedClean) {
                        expect(
                            isOutcomeClean,
                            `ACT Rule expected no failure (was: ${tc.expected}), but Alfa reported a violation (failed)`
                        ).to.be.true
                    }
                })
        })
